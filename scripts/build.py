...
reconstructions/YYYY-MM-DD[_N].txt holds hand-picked good solves (one solve per
...
def fast_solves_page():
    files = sorted((ROOT / "reconstructions").glob("*.txt"), key=file_key, reverse=True)
    L = ["# Fast solve bank", "",
...
def readme(history, n_fast=0):
    L = ["# 3x3 PBs", "",
         ...
         "New here? [What's in this repo →](REPO_GUIDE.md)",
         "",
         "## Current PBs", "",
         "| Event | PB | Date | Stats |", "|---|---|---|---|"]
    for ev in EVENTS:
        if history.get(ev):
            ...
            L.append(f"| **{ev}** | **{fmt(cur['average'])}** | {cur['date']} | "
                     f"[details]({cur['page']}) |")
    L += ["", f"**[Fast solve bank →](reconstructions/README.md)** ({n_fast} solve{'s' if n_fast != 1 else ''})", "",
...
def main():
    ...
    (ROOT / "reconstructions").mkdir(exist_ok=True)
    page, n_fast = fast_solves_page()
    (ROOT / "reconstructions" / "README.md").write_text(page)
